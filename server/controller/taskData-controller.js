...
const createTask = async (req, res) => {
    try {
      ...
      if (!user) {
        return res.status(401).json({ message: 'Forbidden: Only Super Admin or Admin can create tasks.' });
      }
      ...
      if (!['SuperAdmin', 'Admin'].includes(createdByRole)) {
        return res.status(401).json({ message: 'You are not authorized to create tasks.' });
      }
  ...
  const assignTask = async (req, res) => {
    try {
      ...
      // If no user is found in either collection
      if (!user) {
        return res.status(401).json({ message: 'Forbidden: Only Super Admin or Admin can assign tasks.' });
      }
      ...
      if (task.createdByRole === 'SuperAdmin' && userRole === 'SuperAdmin') {
        ...
        if (!admin) {
          return res.status(400).json({ message: 'Admin not found with the provided name.' });
        }
      ...
      if (task.createdByRole === 'Admin' && userRole === 'Admin') {
        ...
        const employee = await Employee.findOne({ fullName: assignedToName, department: task.currentDepartment });
        if (!employee) {
          return res.status(400).json({ message: 'Employee not found in the current department with the provided name.' });
        }
      ...
      return res.status(401).json({ message: 'You are not authorized to assign this task.' });
    } catch (error) {
