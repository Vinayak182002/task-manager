              ...
              {tasks.length > 0 ? (
                tasks.map((task) => (
                  ...
                  >
                    ...
                    <td>
                      {
                        task.departmentDeadlines
                          .filter(
                            (deadline) =>
                              deadline.department === "application"
                          )
                          .map((deadline) =>
                    ...
                    <td>
                      {
                        task.departmentDeadlines
                          .filter(
                            (deadline) =>
                              deadline.department === "production"
                          )
                          .map((deadline) =>
                            new Date(deadline.deadline).toLocaleDateString()
                          )
                    ...
                    <td>
                      {
                        task.departmentDeadlines
                          .filter(
                            (deadline) =>
                              deadline.department === "maintenance"
                          )
                          .map((deadline) =>
                            new Date(deadline.deadline).toLocaleDateString()
                          )
                    ...
                    <td>{task.submissionFiles.filePath}</td>
                    <td>{task.priority}</td>
                  </tr>
                ))
              ) : (
      ...
      {showModal && selectedTask && (
        <div className={styles.modal}>
          <div className={styles.modalContent}>
            ...
            <p><strong>Current Department:</strong> {selectedTask.currentDepartment}</p>
            <p><strong>Next Department:</strong> {selectedTask.nextDepartment}</p>
            <p><strong>Assigned Date:</strong> {new Date(selectedTask.createdAt).toLocaleDateString()}</p>
            <p><strong>Confirm Date from Application Department:</strong> 
              {selectedTask.departmentDeadlines
                .filter((deadline) => deadline.department === "application")
                .map((deadline) => new Date(deadline.deadline).toLocaleDateString())
              }
            </p>
            <p><strong>Confirm Date from Design Department:</strong> 
              {selectedTask.departmentDeadlines
                .filter((deadline) => deadline.department === "design")
                .map((deadline) => new Date(deadline.deadline).toLocaleDateString())
              }
            </p>
            <p><strong>Confirm Date from Production Department:</strong> 
              {selectedTask.departmentDeadlines
                .filter((deadline) => deadline.department === "production")
                .map((deadline) => new Date(deadline.deadline).toLocaleDateString())
              }
            </p>
            <p><strong>Confirm Date from Store Department:</strong> 
              {selectedTask.departmentDeadlines
                .filter((deadline) => deadline.department === "store")
                .map((deadline) => new Date(deadline.deadline).toLocaleDateString())
              }
            </p>
            <p><strong>Confirm Date from Quality Department:</strong> 
              {selectedTask.departmentDeadlines
                .filter((deadline) => deadline.department === "quality")
                .map((deadline) => new Date(deadline.deadline).toLocaleDateString())
              }
            </p>
            <p><strong>Confirm Date from Purchase Department:</strong> 
              {selectedTask.departmentDeadlines
                .filter((deadline) => deadline.department === "purchase")
                .map((deadline) => new Date(deadline.deadline).toLocaleDateString())
              }
            </p>
            <p><strong>Confirm Date from Maintenance Department:</strong> 
              {selectedTask.departmentDeadlines
                .filter((deadline) => deadline.department === "maintenance")
                .map((deadline) => new Date(deadline.deadline).toLocaleDateString())
              }
            </p>
            <p><strong>Confirm Date from Services Department:</strong> 
              {selectedTask.departmentDeadlines
                .filter((deadline) => deadline.department === "services")
                .map((deadline) => new Date(deadline.deadline).toLocaleDateString())
              }
            </p>
            <p><strong>Attachments:</strong> {selectedTask.submissionFiles.filePath}</p>
